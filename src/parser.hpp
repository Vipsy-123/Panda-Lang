...
public:
    ...
    std::optional<NodeString*> parse_string() 
    {   
        
        std::vector<Token> tok; // Temporary vector to store Tokens
        try_consume_err(TokenType::string); // Consumed "
        while (peek().has_value() && peek().value().type == TokenType::string) {
            auto token = try_consume(TokenType::string);
            // std::cout << "HERE"<<"\n";
            if (token.has_value()) {
                tok.push_back(token.value()); // Push the Token value into the vector
            } else {
                // Handle error or unexpected condition
                return std::nullopt; // Return an empty optional
            }
        }
        NodeString* str_vec = m_allocator.emplace<NodeString>(tok); // Create NodeString with the concatenated Tokens
        no_Prints++;
        return str_vec;
    }
    ...
    std::optional<NodeStmt*> parse_stmt() // NOLINT(*-no-recursion)
    {
        ...
        if (peek().has_value() 
            ...
            consume();
            if (const auto expr2 = parse_expr()) {
                stmt_let->expr = expr2.value();
            }
            else {
                error_expected("expression");
            }
        ...
        {
            consume();
            auto prnt = m_allocator.alloc<NodeStmtPrint>();
            ...
            if(auto str = parse_string()){
                ...
                error_expected(" Wrong value Error");  
            }
        ...
        if(peek().has_value() && peek().value().type == TokenType::println) 
        {
            consume();
            auto prnt = m_allocator.alloc<NodeStmtPrint>();
            if (try_consume(TokenType::open_paren));
