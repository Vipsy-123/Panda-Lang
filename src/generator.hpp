...
class Generator {
public:
    explicit Generator(NodeProg prog,std::map<std::string, size_t>& val)
        : m_prog(prog),m_values(val)
    {   
        if(no_Prints > 0){
            gen_data_section();
        }
    ...
    void gen_stmt(const NodeStmt* stmt)
    {
        struct StmtVisitor {
            ...
            {   
                ...
                gen.m_output << "    mov rsi, text" << gen.count << "\n" ;
                gen.m_output << "    mov rdx, textLen" << gen.count << "\n";
                gen.m_output << "    syscall" << "\n";
                gen.count++;
            }
        };
    ...
    std::string gen_data_section() {
        int var = 0;
        m_output << "section .data\n";
       for(auto it : m_values) { 
            m_output << "    " << "text" << var << ":     db " << "'" << it.first << "', 10\n";
            m_output << "    " << "textLen" << var << ":  equ  $-" << "text" << var <<"\n";
            var++; 
        }
        m_output << "\n";
        return m_output.str();
