    ...
    std::vector<Token> tokenize()
    {
        ...
        while (peek().has_value()) {
            if (std::isalpha(peek().value())) {
                ...
                std::string str = " ";
                consume();
                tokens.push_back({TokenType::string, line_count});
                size_t len = 0;
                try 
                {
                    while(peek().has_value() && peek().value() != '"')
                    {
                        if (std::isspace(peek().value())) {
                            ...
                            std::cerr << "This Token is not allowed in String type" << std::endl;
                            exit(EXIT_FAILURE);
                        }
                    }
                    // printf("Exit");
                    m_values.insert({str,len});
                    consume();
                    tokens.push_back({TokenType::string , line_count});
                }
                catch (...) 
                {
                    std::cerr << "Default Exception in string tokenization\n";
                }
            }
            else if (peek().value() == '=') {
                consume();
